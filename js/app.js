...
function initGame() {
  ...
  // Make sure all DOM elements are properly referenced
  if (!timerCountElement) timerCountElement = document.getElementById('timer-count');
  if (!timerBarElement) timerBarElement = document.getElementById('timer-bar');
  if (!tabooWarningElement) tabooWarningElement = document.getElementById('taboo-warning');
...
function setupTabooWordDetection() {
  hintInputElement.addEventListener('input', function() {
    ...
    for (const tabooWord of tabooWords) {
      if (inputText.includes(tabooWord)) {
    ...
    if (containsTabooWord) {
      ...
      submitHintButton.disabled = true; // Disable submit button
      ...
      submitHintButton.disabled = false; // Enable submit button
...
function updateTabooWordsUnlocking() {
  ...
  // Get all taboo word elements
  const tabooWordElements = Array.from(tabooWordsElement.querySelectorAll('.taboo-word'));
  ...
  for (let i = 0; i < unlockingTabooWords.length; i++) {
    if (i < wordsToUnlock) {
      ...
      if (wordElement && wordElement.classList.contains('forbidden')) {
