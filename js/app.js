...
function resetFeedback() {
    ...
    setTimeout(() => {
        ...
        // Correct guess
        const response = getRandomResponse(gameData.aiResponses.guessed).replace('{word}', target);
        addAIMessage(response);
        ...
        const response = getRandomResponse(gameData.aiResponses.incorrect).replace('{guess}', alternativeGuess);
        addAIMessage(response);
    ...
    messageElement.innerHTML = getRandomResponse(gameData.aiResponses.thinking) + '<div class="dot"></div><div class="dot"></div><div class="dot"></div>';
